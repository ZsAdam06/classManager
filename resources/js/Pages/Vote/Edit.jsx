...
export default function Edit({ auth, vote }) {
    const { data, setData, put, processing, errors } = useForm({
        question: vote.question || "",
        ans: vote.ans || "",
        deadline: vote.deadline || "",
        is_finished: vote.is_finished || false,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('votes.update', vote.id));
    }
    return (
        <AuthenticatedLayout
            user={auth.user}
                    ...
                    <div className="p-5 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <form onSubmit={handleSubmit} encType='multipart/form-data'>
                            <div className='flex flex-col space-y-8 text-left'>
                                <h1 className='mx-auto text-xl'>Szavazás szerkesztése</h1>
                                <label>
                                    Kérdés
                                    <textarea 
                                        value={data.question}
